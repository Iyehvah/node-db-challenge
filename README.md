...
- [ SQL is used to QUERY the RELATIONAL DATABASE. ] Explain the difference between `Relational Databases` and `SQL`.

- [ Because we need a way to identify rows in a table ] Why do tables need a `primary key`?

- [ Foreign keys ] What is the name given to a table column that references the primary key on another table.

- [ A table that connects 2 tables together ] What do we need in order to have a _many to many_ relationship between two tables.
